...
class TauriPort implements ISerialPort {
    ...
    async open(options: SerialOptions): Promise<void> {
        ...
        await TauriSerialAPI.openPort({
            ...
            flowControl: (options.flowControl ?? 'none') as 'none' | 'hardware' | 'software',
...
class TauriProvider implements ISerialProvider {
    ...
    isSupported() {
        ...
        return typeof __TAURI_ENV_TARGET_TRIPLE__ !== 'undefined' && !!__TAURI_ENV_TARGET_TRIPLE__;
    }
    ...
    async requestPort(): Promise<ISerialPort | null> {
        ...
        console.warn('TauriProvider: requestPort() not supported. Use getPorts() and select from list.');
        return null;
    }
...
function createSerialProvider(): ISerialProvider {
    ...
    if (typeof __TAURI_ENV_TARGET_TRIPLE__ !== 'undefined' && __TAURI_ENV_TARGET_TRIPLE__) {
        ...
        console.warn('No serial provider available. Falling back to WebSerial (will fail)');
        return new WebSerialProvider();
    }
}
