...
export const composeFrames = (
    ...
    if (strategy === 'SCRIPT' && config.script) {
        try {
            ...
            if (result && Array.isArray(result.frames) && Array.isArray(result.remaining)) {
                return result as CompositionResult;
            } else {
                console.warn("Script framing returned invalid structure. Expected { frames: [], remaining: [] }");
    ...
    while (scanIndex < buffer.length && loops < MAX_LOOPS) {
        ...
        if (frameLength > 0) {
            ...
            for(const c of chunks) {
                if (scanIndex >= currentGlobalIndex && scanIndex < currentGlobalIndex + c.data.length) {
...
function findFrameInView(buf: Uint8Array, config: FramingConfig): { frameLength: number, headerLength: number } {
    const strategy = config.strategy as FramingStrategy;
    ...
    if (strategy === 'PREFIX_LENGTH') {
        ...
        } catch { return { frameLength: 0, headerLength: 0 }; }
        ...
        if (isHex) {
            ...
            for (let i = 0; i < clean.length; i+=2) delimiterBytes[i/2] = parseInt(clean.substring(i, i+2), 16);
        } else {
            const unescaped = dStr.replace(/\\n/g, '\n').replace(/\\r/g, '\r').replace(/\\t/g, '\t');
        ...
        for (let i = 0; i <= buf.length - delimiterBytes.length; i++) {
            ...
            if (match) return { frameLength: i + delimiterBytes.length, headerLength: delimiterBytes.length };
        }
    }
