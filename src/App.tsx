...
const App: React.FC = () => {
  ...
  const {
      ...
      (chunk: Uint8Array, sessionId: string) => {
        ...
        const handleData = framing.handleDataReceived(sessionId, validation.checkValidation);
        handleData(chunk);
      },
    ...
    return (
      <div className="h-full flex items-center justify-center bg-background text-foreground p-4">
        <div className="max-w-md text-center">
          ...
          <h1 className="text-2xl font-bold mb-2">Serial Communication Not Available</h1>
          <p className="text-muted-foreground">
            Please use a Tauri desktop app or a WebSerial-compatible browser (Chrome, Edge, Opera).
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex h-screen w-full min-w-225 min-h-150 bg-background text-foreground font-sans overflow-hidden">
      <Sidebar 
        onSendCommand={(cmd) => commandExecution.handleSendCommandRequest(cmd, (c) => setPendingParamCommand(c))}
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-background relative">
        ...
        <ControlPanel 
            onConnect={async (port) => {
                try {
                    ...
                    console.error(e);
                    addToast('error', 'Connection Error', errorMsg || 'Failed to connect');
                    addSystemLog('ERROR', 'CONNECTION', `Connection failed: ${errorMsg}`);
                }
            }}
            onDisconnect={async () => {
                try {
        ...
        <div className="flex-1 relative overflow-hidden flex flex-row bg-muted/5">
            ...
            <div className="flex-1 flex flex-col min-w-0 relative h-full bg-linear-to-br from-background via-background to-muted/10">
                ...
                <InputPanel
                    onSend={(data) => { commandExecution.sendData(data).catch(() => {}); }}
                    rts={rts} dtr={dtr} 
                    onToggleSignal={(s) => {
      ...
      {pendingParamCommand && (
          <ParameterInputModal 
            ...
            onSend={(values) => {
                ...
                commandExecution.sendData(finalData, pendingParamCommand, values).catch(() => {});
            }}
