...
const defaultElement: MessageElement = {
  id: "",
  name: "",
  size: 1,
...
export const ElementEditModal: React.FC<ElementEditModalProps> = ({
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<MessageElement>({
    resolver: zodResolver(MessageElementSchema),
    defaultValues: element
      ? { ...element }
      : { ...defaultElement, id: generateId() },
  });
  ...
  const toggleIncludeElement = (elementId: string) => {
    ...
    setValue("config", {
      ...editState.config,
      includeElements: newInclude,
    } as ElementConfig);
  };

  const onSubmit = (data: MessageElement): void => {
    onSave(data);
  };
      ...
      <div className="bg-background rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Name</Label>
                <Input {...register("name")} placeholder="field_name" />
                {errors.name && (
              ...
              <Select
                ...
                onChange={(e) =>
                  handleConfigTypeChange(
                    e.target.value as ElementConfig["type"],
                  )
                }
              >
                  ...
                  <div className="space-y-2">
                    ...
                    {otherElements.length === 0 ? (
                      <p className="text-xs text-muted-foreground p-2 bg-muted/50 rounded">
                        Add other elements to the structure first, then
                        configure which ones to include.
                      </p>
                    ) : (
          ...
          <div className="flex justify-end gap-2 p-4 border-t border-border sticky bottom-0 bg-background">
            ...
            <Button type="submit" disabled={!editState.name.trim()}>
              {isNew ? "Add" : "Save"}
            </Button>
          </div>
        </form>
