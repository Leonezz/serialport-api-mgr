...
import { SavedCommand, SerialSequence, DataMode, ValidationMode, TextEncoding, ChecksumAlgorithm } from '../types';
...
export function useCommandExecution(
  ...
  overrideTimerRef: React.MutableRefObject<ReturnType<typeof setTimeout> | null>
) {
  const {
    sessions,
    setConfig,
  ...
  const sendData = async (
    ...
    if (cmdInfo?.scripting?.enabled && cmdInfo.scripting.preRequestScript) {
      try {
        ...
        addSystemLog('ERROR', 'SCRIPT', `Pre-request script failed for ${cmdInfo.name}`, {
          error: errorMsg,
          arguments: { params }
        });
        console.groupEnd();
    ...
    try {
      ...
      addSystemLog('ERROR', 'COMMAND', `Send failed: ${errorMsg}`, { error: String(e) });
      console.groupEnd();
      throw e;
    }
  };
   ...
   */
  const handleSendCommandRequest = (cmd: SavedCommand, onShowParamModal: (cmd: SavedCommand) => void) => {
  ...
  const handleRunSequence = async (seq: SerialSequence) => {
    ...
    for (let i = 0; i < seq.steps.length; i++) {
      ...
      try {
        ...
        const errorMsg = getErrorMessage(e);
        addToast('error', 'Sequence Step Failed', `Step ${i + 1} (${cmd.name}): ${errorMsg}`);
        addSystemLog('ERROR', 'COMMAND', `Sequence ${seq.name} failed at step ${i + 1}`, {
          error: errorMsg
        });
        if (step.stopOnError) {
