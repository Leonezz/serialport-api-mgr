...
export const useSerialConnection = (
    ...
    const readersRef = useRef<Map<string, ReadableStreamDefaultReader<Uint8Array>>>(new Map());
    // Map SessionID -> KeepReading Flag
    const keepReadingRef = useRef<Map<string, boolean>>(new Map());

    const [isSerialSupported, setIsSerialSupported] = useState(serialService.isSupported());
    ...
    const disconnect = async (sessionId: string) => {
        ...
        if (reader) {
            try { await reader.cancel(); } catch (e) { console.warn('Reader cancel error', e); }
            readersRef.current.delete(sessionId);
        }
        
        const port = portsRef.current.get(sessionId);
        if (port) {
            try { await port.close(); } catch (e) { console.warn('Port close error', e); }
    ...
    const connect = async (
        ...
        try {
            ...
            if (typeof requestedPort === 'string' && requestedPort.startsWith('mock')) {
    ...
    const write = async (sessionId: string, data: Uint8Array) => {
        const port = portsRef.current.get(sessionId);
        if (!port || !port.writable) throw new Error('Port not connected or not writable');
    ...
    const toggleSignal = async (sessionId: string, signal: 'rts' | 'dtr') => {
        ...
        if ('getSignals' in port && typeof (port as any).getSignals === 'function') {
             try {
                 const signals = await (port as ISerialPort).getSignals();
                 if (signal === 'rts') await port.setSignals?.({ requestToSend: !signals.requestToSend });
                 if (signal === 'dtr') await port.setSignals?.({ dataTerminalReady: !signals.dataTerminalReady });
             } catch (e) {
    ...
    useEffect(() => {
        ...
        if (serialService.isSupported()) {
            ...
            return () => serialService.removeEventListener('disconnect', onSerialDisconnect);
        }
    }, []);
