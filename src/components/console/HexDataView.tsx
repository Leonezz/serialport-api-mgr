...
const HexDataView: React.FC<HexDataViewProps> = React.memo(({ 
    ...
    const handleInteraction = useCallback((e: React.MouseEvent, type: 'down' | 'over') => {
    ...
    const inspectorData = useMemo(() => {
        ...
        const view = new DataView(selectedBytes.buffer, selectedBytes.byteOffset, selectedBytes.byteLength);
        const res: any = {
            binary: Array.from(selectedBytes).map(b => (b as number).toString(2).padStart(8, '0')).join(' '),
    ...
    const renderRow = (rowIndex: number) => {
        ...
        return (
            <tr key={rowIndex} className="group hover:bg-muted/10 transition-colors">
                {/* Offset - Sticky Left */}
                <td 
                    className={cn(
                        "align-top py-0.5 pr-3 text-muted-foreground/50 border-r border-border/10 select-none text-right font-mono text-[10px]",
                        stickyHeader && "sticky left-0 z-10 bg-background/95 backdrop-blur-[1px]" // Sticky Left + BG
                    )}
                ...
                {/* Hex */}
                <td className="align-top px-3 whitespace-nowrap select-none" style={{ width: hexColWidth, minWidth: hexColWidth }}>
                            ...
                            return (
                                <span 
                                    ...
                                    className={cn(
                                        ...
                                        sel ? "bg-primary text-primary-foreground font-bold" : "text-muted-foreground hover:text-foreground hover:bg-muted/50",
                                        (i > 0 && i % 4 === 0) ? "ml-2" : "ml-1",
                                        i === 0 && "ml-0"
                                    )}
                                    style={{ width: '20px' }}
                ...
                {/* Binary */}
                {!hideBinary && (
                    <td className="align-top px-3 border-l border-border/10 whitespace-nowrap select-none" style={{ width: binColWidth, minWidth: binColWidth }}>
                                ...
                                return (
                                    <span 
                                        ...
                                        className={cn(
                                            ...
                                            sel ? "bg-primary text-primary-foreground font-bold" : "text-muted-foreground/70 hover:text-foreground hover:bg-muted/50",
                                            (i > 0 && i % 4 === 0) ? "ml-2" : "ml-1",
                                            i === 0 && "ml-0"
                                        )}
                                    >
                ...
                {/* ASCII */}
                <td className="align-top px-3 border-l border-border/10 whitespace-nowrap select-none" style={{ width: asciiColWidth, minWidth: asciiColWidth }}>
        ...
        <div className="flex min-h-full w-fit gap-0 bg-background/50 text-[11px]">
            ...
            >
                <table className="w-max table-fixed border-collapse">
                    <thead className={cn("bg-card text-[10px] text-muted-foreground font-medium text-left shadow-sm", stickyHeader && "sticky top-0 z-30")}>
                        <tr className={HEADER_HEIGHT_CLASS}>
                            ...
                            <th className="px-3 font-normal select-none align-middle" style={{ width: hexColWidth }}>
                            ...
                            {!hideBinary && <th className="px-3 border-l border-border/10 font-normal select-none align-middle" style={{ width: binColWidth }}>BINARY</th>}
                            <th className="px-3 border-l border-border/10 font-normal select-none align-middle" style={{ width: asciiColWidth }}>ASCII</th>
                        </tr>
                    </thead>
                    <tbody>
                        {(Array.from as any)({ length: rowCount }).map((_: any, i: number) => renderRow(i))}
                    </tbody>
                </table>
            </div>
            ...
            {showInspector && (
                <div className={cn(
                    ...
                    stickyHeader && "sticky right-0 z-20 shadow-[linear-gradient(to_right,rgba(0,0,0,0),rgba(0,0,0,0.1))]" // Sticky Right
                )}>
                    {/* Sticky Wrapper for Inspector Content */}
                    <div className={cn("flex flex-col w-full", stickyHeader && "sticky top-0 max-h-screen")}>
                        <div className={cn(
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                />
                                <label htmlFor="le-chk" className="cursor-pointer select-none">Little Endian</label>
                            </div>
                        </div>
                            ...
                            {inspectorData ? (
                                <>
                                    <div className="space-y-1">
                                        <div className="text-muted-foreground text-[9px] uppercase tracking-wider">Selection</div>
                                        <div className="bg-background border border-border p-1.5 rounded truncate">
                                            {selection ? `${selection.start} - ${selection.end} (${inspectorData.size} bytes)` : '-'}
                                        </div>
                                    </div>

                                    <div className="space-y-1">
                                        <div className="text-muted-foreground text-[9px] uppercase tracking-wider">Binary</div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {inspectorData.int8 !== undefined && (
                                            <div><div className="text-muted-foreground opacity-70">Int8</div><div className="font-semibold">{inspectorData.int8}</div></div>
                                        )}
                                        {inspectorData.uint8 !== undefined && (
                                            <div><div className="text-muted-foreground opacity-70">Uint8</div><div className="font-semibold">{inspectorData.uint8}</div></div>
                                        )}
                                        {inspectorData.int16 !== undefined && (
                                            <div><div className="text-muted-foreground opacity-70">Int16</div><div className="font-semibold">{inspectorData.int16}</div></div>
                                        )}
                                        {inspectorData.uint16 !== undefined && (
                                            <div><div className="text-muted-foreground opacity-70">Uint16</div><div className="font-semibold">{inspectorData.uint16}</div></div>
                                        )}
                                        {inspectorData.int32 !== undefined && (
                                            <div><div className="text-muted-foreground opacity-70">Int32</div><div className="font-semibold">{inspectorData.int32}</div></div>
                                        )}
                                        {inspectorData.uint32 !== undefined && (
                                            <div><div className="text-muted-foreground opacity-70">Uint32</div><div className="font-semibold">{inspectorData.uint32}</div></div>
                                        )}
                                        {inspectorData.float32 !== undefined && (
                                            <div className="col-span-2"><div className="text-muted-foreground opacity-70">Float32</div><div className="font-semibold">{inspectorData.float32}</div></div>
                                        )}
                                        {inspectorData.int64 !== undefined && (
                                            <div className="col-span-2"><div className="text-muted-foreground opacity-70">Int64</div><div className="font-semibold truncate">{inspectorData.int64.toString()}</div></div>
                                        )}
                                        {inspectorData.float64 !== undefined && (
                                            <div className="col-span-2"><div className="text-muted-foreground opacity-70">Float64</div><div className="font-semibold truncate">{inspectorData.float64}</div></div>
                                        )}
                                    </div>
                                </>
                            ) : (
                                <div className="flex flex-col items-center justify-center text-muted-foreground/40 space-y-2 select-none py-6">
                                    ...
                                    <p className="text-center font-sans text-xs">Select bytes<br/>to inspect</p>
                                </div>
                            )}
                        </div>
                    </div>
