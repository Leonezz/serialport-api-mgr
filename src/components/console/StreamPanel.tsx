...
const StreamPanel: React.FC<StreamPanelProps> = ({ title, logs, direction, className, accentColorClass }) => {
    ...
    return (
        <div className={cn("flex flex-col h-full border rounded-lg overflow-hidden bg-card shadow-sm relative", className)}>
             {/* Header */}
             <div className={cn("flex items-center justify-between px-3 py-2 border-b text-xs font-bold tracking-wider shrink-0", accentColorClass || "bg-muted/30")}>
                <div className="flex items-center gap-2">
                    {direction === 'RX' ? <ArrowDown className="w-3.5 h-3.5" /> : <ArrowDown className="w-3.5 h-3.5 rotate-180" />}
                    {title}
                </div>
                <div className="flex items-center gap-4">
                     ...
                     <span className="font-mono opacity-70 text-[10px]">{combinedData.length} B</span>
                </div>
             </div>
