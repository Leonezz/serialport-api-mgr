...
import { LayoutDashboard, Trash2, Move, ArrowLeftRight, Settings, Download, Gauge, X, Save, Maximize2, Minimize2, Activity, Plus, Link as LinkIcon, Edit3 } from 'lucide-react';
...
const DashboardPanel: React.FC = () => {
    const { sessions, activeSessionId, clearVariables, reorderWidgets, addWidget, updateWidget, removeWidget } = useStore();
    ...
    const [brushTimeRange, setBrushTimeRange] = useState<{ startTime?: number; endTime?: number }>({});
        ...
        <div className="flex flex-col h-full bg-muted/10 p-4 overflow-hidden relative">
            <div className="flex justify-between items-center mb-4 shrink-0">
                ...
                <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => setIsAddModalOpen(true)} className="h-8 gap-1 text-xs">
                        <Plus className="w-3.5 h-3.5" /> Add Widget
                    </Button>
                    {widgets.length > 0 && (
                        <Button variant="ghost" size="icon" onClick={clearVariables} className="text-muted-foreground hover:text-destructive h-8 w-8" title="Clear All Widgets">
            ...
            {widgets.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full bg-card/50 text-muted-foreground select-none">
                    ...
                    <p className="text-sm opacity-70 max-w-sm text-center">
                        ...
                        Variables from scripts will auto-create widgets, or you can add them manually.
                    </p>
...
const WidgetCard: React.FC<{ 
    ...
    onMaximize: () => void;
    isMaximized: boolean;
}> = ({ widget, variable, globalMinTime, globalMaxTime, onDragStart, onDragOver, onDrop, isDragging, brushTimeRange, setBrushTimeRange, onEdit, onDelete, onMaximize, isMaximized }) => {
    const { updateWidget } = useStore();
    const config = widget.config;
    
    // Width class is irrelevant if maximized
    const widthClass = isMaximized ? "w-full" : (config.width === 3 ? "w-full" : config.width === 2 ? "w-[calc(66.66%-0.7rem)]" : "w-[calc(33.33%-0.7rem)] min-w-[280px]");
    
    // Fallback dummy variable if real one missing to prevent crash, but show empty state
    const safeVar = variable || { name: widget.variableName, type: 'string', value: 'No Data', lastUpdate: 0, history: [] };
    const isBool = safeVar.type === 'boolean';
    const isObj = safeVar.type === 'object';
    const isNum = safeVar.type === 'number';
    const timeStr = variable ? new Date(variable.lastUpdate).toLocaleTimeString() : '--:--:--';

    const toggleWidth = (e: React.MouseEvent) => {
        e.stopPropagation();
        const nextWidth = (config.width % 3) + 1;
        updateWidget(widget.id, { config: { ...config, width: nextWidth as 1|2|3 } });
    };
    ...
    const nextType = (e: React.MouseEvent) => {
        ...
        if (config.type === 'CARD') {
            ...
            if (isNum) updateWidget(widget.id, { config: { ...config, type: 'GAUGE' } });
    ...
    const handleExport = (e: React.MouseEvent) => {
        ...
        const dataToExport = variable.history.filter(h => h.time >= start && h.time <= end);
    ...
    const processedData = useMemo(() => {
        ...
        if (globalMinTime === undefined || globalMaxTime === undefined) return safeVar.history;
    ...
    const brushIndices = useMemo(() => {
        ...
        if (brushTimeRange.startTime === undefined && brushTimeRange.endTime === undefined) return {};

        let startIndex = 0;
        let endIndex = processedData.length - 1;

        if (brushTimeRange.startTime !== undefined) {
            const idx = processedData.findIndex(h => h.time >= brushTimeRange.startTime!);
            if (idx !== -1) startIndex = idx;
        }

        if (brushTimeRange.endTime !== undefined) {
            const idx = processedData.findIndex(h => h.time > brushTimeRange.endTime!);
    ...
    const handleBrushChange = useCallback((e: any) => {
        ...
        if (e.startIndex !== undefined && e.endIndex !== undefined) {
            ...
            if (startItem && endItem) {
                setBrushTimeRange({ startTime: startItem.time, endTime: endItem.time });
            }
        }
    ...
    const renderContent = () => {
        ...
        if (config.type === 'LINE') {
            const keys = isObj && safeVar.history.length > 0 
                ? Object.keys(safeVar.history[0]).filter(k => k !== 'time') 
                : ['val'];

            if (safeVar.type === 'string') {
                return <div className="flex items-center justify-center h-full text-amber-500 text-xs">String data cannot be plotted on Line Chart.</div>;
            }
    ...
    return (
        <div 
            ...
            className={cn(
                ...
                isMaximized ? "h-full w-full border-0 shadow-none" : `h-[200px] ${widthClass}`,
                isDragging ? "opacity-50 border-dashed border-primary" : (isMaximized ? "" : "border-border"),
                (!isMaximized && isBool && safeVar.value) ? "border-emerald-500/50 bg-emerald-500/5" : ""
            )}
        >
            {/* Header / Controls */}
            <div className="flex justify-between items-start p-3 pb-0 relative z-10">
                <div className="flex items-center gap-2 overflow-hidden">
                    ...
                    <div>
                        <h4 className={cn("font-bold text-muted-foreground uppercase tracking-wide truncate flex items-center gap-1.5", isMaximized ? "text-xl max-w-full" : "text-xs max-w-[120px]")} title={widget.title}>
                            {widget.title}
                            {widget.title !== widget.variableName && (
                                <span className="text-[9px] font-normal opacity-50 font-mono">({widget.variableName})</span>
                            )}
                        </h4>
                        <span className="text-[9px] text-muted-foreground opacity-50 block">{timeStr}</span>
                    </div>
                </div>
                
                <div className={cn("flex items-center gap-1 transition-opacity bg-background/80 backdrop-blur rounded-md border border-border shadow-sm p-0.5", !isMaximized && "opacity-0 group-hover:opacity-100")}>
                    <button onClick={handleExport} className="p-1 hover:bg-muted rounded text-muted-foreground hover:text-foreground" title="Export CSV">
                        <Download className="w-3 h-3" />
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="p-1 hover:bg-muted rounded text-muted-foreground hover:text-foreground" title="Widget Settings">
                        <Settings className="w-3 h-3" />
                    </button>
                    <button onClick={nextType} className="p-1 hover:bg-muted rounded text-muted-foreground hover:text-foreground" title="Switch View">
                        {config.type === 'CARD' ? <Activity className="w-3 h-3" /> : config.type === 'LINE' ? <Gauge className="w-3 h-3" /> : <LayoutDashboard className="w-3 h-3" />}
                    </button>
                    {!isMaximized && (
                        <button onClick={toggleWidth} className="p-1 hover:bg-muted rounded text-muted-foreground hover:text-foreground" title="Resize">
                            <ArrowLeftRight className="w-3 h-3" />
                        </button>
                    )}
                    <button onClick={(e) => { e.stopPropagation(); onMaximize(); }} className="p-1 hover:bg-muted rounded text-muted-foreground hover:text-foreground" title={isMaximized ? "Minimize" : "Maximize"}>
...
const WidgetSettingsModal: React.FC<{ widget: DashboardWidget; availableVariables: string[]; onClose: () => void }> = ({ widget, availableVariables, onClose }) => {
    const { updateWidget } = useStore();
    const config = widget.config;
        ...
        <div className="absolute inset-0 bg-background/50 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
            <Card className="w-[320px] shadow-xl border-border animate-in zoom-in-95 duration-150">
                <div className="flex items-center justify-between p-3 border-b border-border bg-muted/20">
                    <span className="font-bold text-xs flex items-center gap-2"><Edit3 className="w-3 h-3" /> Edit Widget</span>
                ...
                <div className="p-4 space-y-3">
                    ...
                    <div className="space-y-1">
                        ...
                        <Input className="h-8 text-xs" value={unit} onChange={e => setUnit(e.target.value)} placeholder="e.g. Volts, RPM" />
                    </div>

                    <div className="grid grid-cols-2 gap-3 pt-1">
                        <div className="space-y-1">
                            <Label className="text-[10px]">Min Value</Label>
                            <Input className="h-8 text-xs" type="number" value={min} onChange={e => setMin(e.target.value)} />
                        </div>
                        <div className="space-y-1">
                            <Label className="text-[10px]">Max Value</Label>
                            <Input className="h-8 text-xs" type="number" value={max} onChange={e => setMax(e.target.value)} />
                        </div>
                    </div>
                    
                    <Button size="sm" className="w-full h-8 text-xs gap-2 mt-2" onClick={handleSave}>
...
const AddWidgetModal: React.FC<{ onAdd: (data: AddWidgetData) => void; onClose: () => void; availableVariables: string[] }> = ({ onAdd, onClose, availableVariables }) => {
        ...
        <div className="absolute inset-0 bg-background/50 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
            <Card className="w-[320px] shadow-xl border-border animate-in zoom-in-95 duration-150">
                ...
                <form onSubmit={handleSave} className="p-4 space-y-3">
                    ...
                    <div className="space-y-1">
                        <Label className="text-[10px]">Widget Title</Label>
                        <Input className="h-8 text-xs" value={title} onChange={e => setTitle(e.target.value)} placeholder="Display Name" />
                    </div>

                    <div className="space-y-1">
                        <Label className="text-[10px]">Widget Type</Label>
                        <Select className="h-8 text-xs" value={type} onChange={e => setType(e.target.value as any)}>
                    ...
                    <div className="space-y-1">
                        <Label className="text-[10px]">Unit Label</Label>
                        <Input className="h-8 text-xs" value={unit} onChange={e => setUnit(e.target.value)} placeholder="e.g. V, A, °C" />
                    </div>
                    ...
                    <Button type="submit" size="sm" className="w-full h-8 text-xs gap-2 mt-2">
