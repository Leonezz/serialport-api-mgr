...
import { Clock, Binary, Code, Type, Check, Copy, BookOpen, List, Sliders, Palette } from 'lucide-react';
...
const ChatBubble: React.FC<Props> = ({ log, globalFormat, context, enableAnsi = true, isDark = false }) => {
  const [viewMode, setViewMode] = useState<DataMode | 'PARAMS'>(log.format);
  const [copied, setCopied] = useState(false);

  const hasParams = log.commandParams && Object.keys(log.commandParams).length > 0;
  const hasExtractedVars = log.extractedVars && Object.keys(log.extractedVars).length > 0;
  ...
  const content = useMemo(() => {
      ...
      if (typeof log.data !== 'string' && log.payloadStart !== undefined && log.payloadLength !== undefined) {
          // If framed binary, extract the semantic payload for display
          dataToFormat = (log.data as Uint8Array).subarray(log.payloadStart, log.payloadStart + log.payloadLength);
      }
      ...
      return (
          <div className="flex flex-col gap-1.5 mt-1">
              {Object.entries(dataObj).map(([key, value]) => (
                  <div key={key} className="flex items-start text-xs border-b border-white/10 last:border-0 pb-1 last:pb-0">
                      <span className="font-bold opacity-70 w-1/3 truncate text-right pr-3 shrink-0">{key}</span>
                      <span className="font-mono opacity-90 break-all select-all">{typeof value === 'object' ? JSON.stringify(value) : String(value)}</span>
                  </div>
              ))}
          </div>
      );
      ...
      <div className={cn(
        ...
        <div className={cn(
            ...
            <div className="flex items-center gap-2 opacity-70">
                ...
                <span className="text-[9px] font-bold opacity-60 uppercase tracking-widest">{log.direction}</span>
                
                {/* Context Indicator */}
                {context && (
                    <div className="ml-2 flex items-center gap-1 group/ctx relative">
                        <BookOpen className="w-3 h-3" />
                        <span className="text-[9px] font-bold border-b border-dotted cursor-help max-w-[100px] truncate">{context.title}</span>
                        {/* Tooltip */}
                        <div className="absolute left-0 bottom-full mb-2 w-64 p-2 bg-popover border border-border rounded-md shadow-lg text-[10px] text-popover-foreground invisible group-hover/ctx:visible z-50 text-left">
                            <p className="font-bold mb-1 border-b pb-1 opacity-70 truncate">{context.title}</p>
                            <p className="opacity-80 line-clamp-6 whitespace-pre-wrap">{context.content}</p>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="flex items-center gap-1">
                 ...
                 {canShowParams && (
                     <button
                        ...
                        className={cn(
                            ...
                            viewMode === 'PARAMS' 
                                ? (isTx ? "bg-background/20 shadow-sm" : "bg-muted text-foreground shadow-sm")
                                : (isTx ? "hover:bg-background/10 opacity-50 hover:opacity-100" : "hover:bg-muted text-muted-foreground")
                        )}
                        title="View Parameters"
                     >
                        {isTx ? <Sliders className="w-3 h-3" /> : <List className="w-3 h-3" />}
                     </button>
                 )}
                 ...
                 <div className={cn("w-px h-3 mx-1", isTx ? "bg-primary-foreground/20" : "bg-border")}></div>

                 <button 
                    onClick={handleCopy}
                    className={cn(
                        "p-1 rounded transition-colors",
                        isTx ? "hover:bg-background/10" : "hover:bg-muted text-muted-foreground hover:text-foreground"
                    )}
                 >
