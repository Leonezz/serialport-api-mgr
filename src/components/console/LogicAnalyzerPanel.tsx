...
import { Activity, Search, ZoomIn, ZoomOut, RotateCcw, MoveHorizontal, ArrowRightToLine } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, ReferenceLine, Brush } from 'recharts';
...
const LogicAnalyzerPanel: React.FC<{ logs: LogEntry[] }> = ({ logs }) => {
    ...
    const chartData = useMemo(() => {
        ...
        const data: { index: number; rx?: number; tx?: number; byteInfo?: string; timestamp?: string }[] = [];
        let globalIndex = 0;

        // Initial IDLE state (Both channels idle)
        data.push({ index: globalIndex++, rx: RX_IDLE, tx: TX_IDLE, timestamp: '' });
    ...
    const dragStartRef = useRef<{ x: number; domainMin: number; domainMax: number } | null>(null);
    ...
    useEffect(() => {
        ...
        const handleWheelNative = (e: WheelEvent) => {
            ...
            if (e.ctrlKey) {
                ...
                let mousePercent = (e.clientX - rect.left - CHART_LEFT_OFFSET) / chartWidth;
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        if (!isDragging || !dragStartRef.current || !domain || !containerRef.current) return;
        ...
        const currentRange = dragStartRef.current.domainMax - dragStartRef.current.domainMin;
        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="bg-popover border border-border p-2 rounded shadow-lg text-xs font-mono z-50 text-popover-foreground">
                    {time && <p className="font-bold text-muted-foreground mb-1">{time}</p>}
                    <p className="font-bold mb-1 opacity-70">Sequence: {Math.round(label)}</p>
                    
                    {rxVal && (
                        <div className={rxVal.value === RX_IDLE ? "text-muted-foreground opacity-50" : "text-emerald-500"}>
                            RX: {formatValue(rxVal.value, 'RX')} {rxVal.value !== RX_IDLE && info && `(${info})`}
                        </div>
                    )}
                    {txVal && (
                        <div className={txVal.value === TX_IDLE ? "text-muted-foreground opacity-50" : "text-violet-500"}>
    ...
    return (
        <div className="flex flex-col h-full bg-card rounded-lg border border-border overflow-hidden relative group">
            ...
            <div className="absolute top-2 left-4 z-20 flex items-start justify-between w-[calc(100%-2rem)] pointer-events-none">
                <div className="text-[10px] text-muted-foreground font-mono bg-background/80 backdrop-blur px-2 py-1 rounded border border-border/30">
                    <div className="flex items-center gap-2">
                        <span className="w-2 h-2 bg-violet-500 rounded-full inline-block"></span> TX (Upper)
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                        <span className="w-2 h-2 bg-emerald-500 rounded-full inline-block"></span> RX (Lower)
                    </div>
                </div>
                ...
                <div className="flex items-center gap-1 bg-background/80 backdrop-blur p-1 rounded border border-border/30 pointer-events-auto shadow-sm opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => zoomStep('in')} title="Zoom In (Ctrl+Scroll)"><ZoomIn className="w-3.5 h-3.5" /></Button>
                    <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => zoomStep('out')} title="Zoom Out (Ctrl+Scroll)"><ZoomOut className="w-3.5 h-3.5" /></Button>
                    <Button variant="ghost" size="icon" className={cn("h-6 w-6 transition-colors", isAutoScroll && "text-primary bg-primary/10")} onClick={handleReset} title="Jump to Live Data"><ArrowRightToLine className="w-3.5 h-3.5" /></Button>
            ...
            <div 
                ref={containerRef}
                className={cn("flex-1 w-full mt-4 text-xs font-mono select-none", isDragging ? "cursor-grabbing" : "cursor-grab")}
            ...
            >
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <ReferenceLine y={RX_IDLE} stroke="hsl(var(--muted-foreground))" strokeDasharray="3 3" opacity={0.3} strokeWidth={1} />
                        <ReferenceLine y={TX_IDLE} stroke="hsl(var(--muted-foreground))" strokeDasharray="3 3" opacity={0.3} strokeWidth={1} />

                        <XAxis 
                            dataKey="index" 
                            type="number" 
                            domain={domain ? [domain.min, domain.max] : ['dataMin', 'dataMax']}
                        ...
                        >
                            <LineChart data={chartData}>
                                <Line type="stepAfter" dataKey="rx" stroke="#10b981" dot={false} strokeWidth={1} />
                                <Line type="stepAfter" dataKey="tx" stroke="#8b5cf6" dot={false} strokeWidth={1} />
                            </LineChart>
                        </Brush>
                    </LineChart>
                </ResponsiveContainer>
