...
import { SerialConfig, NetworkConfig, SerialPreset, SavedCommand, DashboardWidget, FramingConfig, SerialSequence, ProjectContext } from '../types';
    ...
    <div className="border-b border-border bg-background flex items-center px-4 py-3 gap-5 shrink-0 z-20 shadow-sm relative">
      ...
      <div className="relative flex-1 min-w-0">
        ...
        >
         ...
         {connectionType === 'SERIAL' ? (
             <>
                <div className="flex flex-col gap-1.5"><span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider pl-1">{t('cp.port')}</span><div className="flex items-center gap-1"><div className="w-50"><Select className="h-9 text-xs font-mono bg-muted/30 border-border focus:border-primary focus:ring-primary/20" value={selectedPortIndex} onChange={(e) => setSelectedPortIndex(e.target.value)} disabled={isConnected}><option value="" disabled>Select a port...</option>
                ...
                <div className="flex flex-col gap-1.5"><span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider pl-1">{t('cp.baud')}</span><div className="w-25"><Select value={config.baudRate} onChange={(e) => handleChange('baudRate', parseInt(e.target.value))} className="h-9 text-xs font-mono bg-muted/30 border-border" disabled={isConnected}>{[9600, 19200, 38400, 57600, 74880, 115200, 230400, 460800, 921600].map(r => (<option key={r} value={r}>{r}</option>))}</Select></div></div>
                <div className="flex flex-col gap-1.5"><span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider pl-1">Data Bits</span><div className="w-17.5"><Select value={config.dataBits} onChange={(e) => handleChange('dataBits', parseInt(e.target.value))} className="h-9 text-xs font-mono bg-muted/30 border-border" disabled={isConnected}><option value={8}>8-bit</option><option value={7}>7-bit</option></Select></div></div>
                <div className="flex flex-col gap-1.5"><span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider pl-1">Parity</span><div className="w-20"><Select value={config.parity} onChange={(e) => handleChange('parity', e.target.value)} className="h-9 text-xs font-mono bg-muted/30 border-border" disabled={isConnected}><option value="none">None</option><option value="even">Even</option><option value="odd">Odd</option></Select></div></div>
                <div className="flex flex-col gap-1.5"><span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider pl-1">Stop Bits</span><div className="w-17.5"><Select value={config.stopBits} onChange={(e) => handleChange('stopBits', parseInt(e.target.value))} className="h-9 text-xs font-mono bg-muted/30 border-border" disabled={isConnected}><option value={1}>1-bit</option><option value={2}>2-bit</option></Select></div></div>
         ...
         <div className="flex flex-col gap-1.5">
             ...
             <div className="flex items-center gap-1.5 bg-muted/30 p-1 rounded-md border border-border h-9">
                 {loadedPresetId ? (
                     <>
                        <div className={cn("px-2 text-xs font-medium max-w-30 truncate flex items-center gap-1.5", isPresetDirty ? "text-amber-500" : "text-foreground")}>{activePresetName}{isPresetDirty && <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" title="Unsaved Changes"></span>}</div>
