...
const CommandList: React.FC<Props> = ({ commands, onSend, onDelete }) => {
  return (
    <div className="flex flex-col gap-1">
      ...
      {commands.map((cmd) => (
        <div key={cmd.id} className="p-3 hover:bg-muted/50 rounded-md group transition-colors flex flex-col gap-2 border border-transparent hover:border-border">
          <div className="flex items-center justify-between">
            <span className="font-medium text-foreground text-sm truncate pr-2">{cmd.name}</span>
