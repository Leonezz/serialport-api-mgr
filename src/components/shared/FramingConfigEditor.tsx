    ...
    <div className={spacingClass}>
      ...
      <div className="space-y-1">
        <div className="text-xs font-bold text-muted-foreground">
          {label || 'Strategy'}
          {showPersistence && <span className="text-[9px] ml-2 opacity-60">(Command-specific)</span>}
        </div>
        <Select
          value={strategy}
          onChange={(e) => handleStrategyChange(e.target.value as FramingStrategy)}
      ...
      {strategy === 'PREFIX_LENGTH' && (
        <div className="space-y-3 animate-in slide-in-from-top-2">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input
                ...
                min={1}
                max={8}
                value={config.prefixLengthSize || FRAMING.DEFAULT_PREFIX_LENGTH_SIZE}
                onChange={(e) => onChange({ prefixLengthSize: parseInt(e.target.value) })}
                className="h-9 text-sm"
              />
            </div>
            <div className="space-y-1">
              ...
              <Select
                ...
                onChange={(e) => onChange({ byteOrder: e.target.value as 'LE' | 'BE' })}
                className="h-9 text-sm"
          ...
          <div className="text-[9px] text-muted-foreground opacity-70">
            Expects {config.prefixLengthSize || FRAMING.DEFAULT_PREFIX_LENGTH_SIZE} byte(s) at start, followed by that many bytes of data.
          </div>
        </div>
      )}
