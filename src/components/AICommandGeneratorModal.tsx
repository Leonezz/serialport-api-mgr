...
import { AIProjectResult, generateProjectFromDescription } from '../services/geminiService';
import { X, Sparkles, Check, AlertCircle, Loader2, ArrowRight, Paperclip, FileText, Settings2, Terminal, ListVideo, Box, Trash2, Edit2, Save, AlertTriangle, Sliders, Coins } from 'lucide-react';
...
const AICommandGeneratorModal: React.FC<Props> = ({ existingCommands, initialResult, onImport, onClose }) => {
  const { sequences, contexts, setContexts } = useStore();
  const [input, setInput] = useState('');
  const [isGenerating, setIsGenerating] = useState(false);
  const [generatedResult, setGeneratedResult] = useState<AIProjectResult | null>(null);
  const [deviceName, setDeviceName] = useState('');
  const [error, setError] = useState<string | null>(null);
  const [step, setStep] = useState<'input' | 'preview'>('input');
  const [attachment, setAttachment] = useState<Attachment | null>(null);
  const [previewTab, setPreviewTab] = useState<'config' | 'commands' | 'sequences'>('commands');
  
  // Advanced Edit State
  const [editingCommandIndex, setEditingCommandIndex] = useState<number | null>(null);
  const [editingCommandData, setEditingCommandData] = useState<Partial<SavedCommand> | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const result = await generateProjectFromDescription(input, attachment || undefined);
      
      if (result.commands.length > 0 || result.sequences.length > 0 || Object.keys(result.config || {}).length > 0) {
        ...
        setError("AI couldn't find any relevant configuration in the provided content.");
      }
    } catch (err: any) {
  ...
  const startEditCommand = (index: number) => {
      ...
      setEditingCommandData({ ...generatedResult.commands[index], id: 'temp-edit' }); // Dummy ID for form
  };

  const checkDuplicate = (name: string) => {
      return existingCommands.some(c => c.name.toLowerCase() === name.toLowerCase());
  };
      ...
      <Card className="w-full max-w-4xl shadow-2xl border-border animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh] relative">
        <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-border bg-muted/20">
          ...
          <div className="flex items-center gap-2">
              {generatedResult?.usage && step === 'preview' && (
                  <Badge variant="outline" className="text-[10px] font-mono gap-1 text-muted-foreground bg-background/50">
                      <Coins className="w-3 h-3 text-amber-500" /> {generatedResult.usage.total} Tokens
                  </Badge>
              )}
                ...
                {step === 'input' && (
                    <div className="space-y-4">
                        <div className="p-4 bg-purple-500/5 border border-purple-500/20 rounded-lg text-sm text-foreground/80">
                            ...
                            Paste a device manual or describe your project. 
                            ...
                            <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                                <div>
                                    ...
                                    {!attachment ? (
                                        ...
                                        >
                                            <Paperclip className="w-3.5 h-3.5" /> Attach Manual (PDF/Img)
                                        </Button>
                                    ) : (
                                        <div className="flex items-center gap-2 bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-300 px-2 py-1 rounded-md border border-purple-200 dark:border-purple-800 animate-in fade-in slide-in-from-left-2">
                                            ...
                                            <span className="text-xs font-medium max-w-[200px] truncate" title={attachment.name}>{attachment.name}</span>
                                            <button 
                ...
                {step === 'preview' && generatedResult && (
                    <div className="flex flex-col h-full gap-4">
                        ...
                        <div className="flex flex-col gap-1.5 bg-muted/20 p-3 rounded-md border border-border/50 shrink-0">
                            <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Device Group Name</Label>
                            <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center justify-between shrink-0">
                            <div className="flex items-center gap-2 bg-muted p-1 rounded-md">
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        previewTab === 'commands' ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
                                    )}
                                >
                                    <Terminal className="w-3.5 h-3.5" />
                                    Commands <Badge variant="secondary" className="ml-1 px-1 h-4 text-[9px]">{generatedResult.commands.length}</Badge>
                                </button>
                                <button
                                    onClick={() => setPreviewTab('sequences')}
                                    className={cn(
                                        "px-3 py-1.5 rounded-sm text-xs font-medium transition-all flex items-center gap-2",
                                        previewTab === 'sequences' ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
                                    )}
                                >
                                    <ListVideo className="w-3.5 h-3.5" />
                                    Sequences <Badge variant="secondary" className="ml-1 px-1 h-4 text-[9px]">{generatedResult.sequences.length}</Badge>
                                </button>
                                <button
                                    onClick={() => setPreviewTab('config')}
                                    className={cn(
                                        "px-3 py-1.5 rounded-sm text-xs font-medium transition-all flex items-center gap-2",
                                        previewTab === 'config' ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
                                    )}
                                >
                            ...
                            <Button variant="ghost" size="sm" onClick={() => {
                                if (initialResult) onClose(); // If came from Assistant, just close
                        ...
                        <div className="flex-1 overflow-y-auto border rounded-md bg-muted/10 p-4 custom-scrollbar">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {generatedResult.commands.length === 0 ? (
                                        ...
                                        generatedResult.commands.map((cmd, idx) => {
                                            ...
                                            return (
                                            <div key={idx} className={cn("p-3 rounded-md border border-border bg-card flex flex-col gap-2 shadow-sm relative group transition-all", isDuplicate && "border-amber-500/50 bg-amber-500/5")}>
                                                {/* Actions */}
                                                <div className="absolute right-2 top-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                                    <button onClick={() => startEditCommand(idx)} className="p-1 hover:bg-muted rounded text-muted-foreground hover:text-foreground" title="Edit"><Edit2 className="w-3.5 h-3.5" /></button>
                                                    <button onClick={() => deleteCommand(idx)} className="p-1 hover:bg-destructive/10 rounded text-muted-foreground hover:text-destructive" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                                                </div>

                                                <div className="flex items-center gap-2 pr-12">
                                                    <span className="font-bold text-sm truncate">{cmd.name}</span>
                                                    <Badge variant="outline" className="text-[10px] h-5 opacity-50">{cmd.group || 'General'}</Badge>
                                                    {isDuplicate && (
                                                        <span className="text-[10px] text-amber-600 flex items-center gap-1 bg-amber-100 dark:bg-amber-900/30 px-1.5 rounded-sm"><AlertTriangle className="w-3 h-3" /> Duplicate Name</span>
                                                    )}
                                                </div>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        {cmd.parameters && cmd.parameters.length > 0 && (
                                                            <span className="text-[9px] flex items-center gap-1 text-blue-600 dark:text-blue-400 font-bold">
                                                                <Sliders className="w-3 h-3" /> {cmd.parameters.length} Params
                                                            </span>
                                                        )}
                                                    </div>
                                                    <Badge variant="secondary" className="text-[9px] h-4 ml-auto">{cmd.mode}</Badge>
                                                </div>
                                            </div>
                                        )})
                                    )}
                                ...
                                <div className="space-y-4">
                                     {generatedResult.sequences.length === 0 ? (
                                        ...
                                        generatedResult.sequences.map((seq, idx) => (
                                            <div key={idx} className="p-3 rounded-md border border-border bg-card flex flex-col gap-2 shadow-sm relative group">
                                                <div className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                     <button onClick={() => deleteSequence(idx)} className="p-1 hover:bg-destructive/10 rounded text-muted-foreground hover:text-destructive" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                                                </div>
                                                ...
                                                <div className="space-y-1">
                                                    {(seq.steps || []).map((step, sIdx) => (
                                                        <div key={sIdx} className="flex items-center gap-2 text-xs p-1.5 bg-muted/30 rounded">
                                                            <span className="font-mono text-muted-foreground w-4">{sIdx + 1}.</span>
                                                            <span className="font-medium flex-1">{step.commandName}</span>
                                                            <span className="text-muted-foreground text-[10px]">Wait {step.delay}ms</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                            ...
                            {previewTab === 'config' && (
                                <div className="flex flex-col gap-4">
                                    {Object.keys(generatedResult.config || {}).length === 0 ? (
                                        <div className="text-center py-10 text-muted-foreground opacity-50">
                                            No configuration changes detected.
                                        </div>
                                    ) : (
                                        <div className="grid grid-cols-2 gap-4">
                                            {Object.entries(generatedResult.config || {}).map(([key, value]) => (
                                                <div key={key} className="p-3 rounded-md border border-border bg-card flex flex-col gap-1">
                                                    <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                                    ...
                                    <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 rounded text-xs border border-blue-100 dark:border-blue-900">
                                        Note: These settings will be applied to your current active configuration upon import.
                                    </div>
                                </div>
                            )}
                        </div>
        ...
        {editingCommandData && (
            <CommandFormModal 
                ...
                existingGroups={[...existingCommands, ...(generatedResult?.commands || [])].map(c => c.group).filter(Boolean) as string[]}
                sequences={sequences}
                contexts={contexts}
                onSave={(data) => {
                ...
                onUpdateContext={(ctx) => setContexts(prev => prev.map(c => c.id === ctx.id ? ctx : c))}
            />
        )}
      </Card>
    </div>
