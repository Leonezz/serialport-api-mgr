...
import { Bot, Sliders, FileText, ChevronsRight, ChevronsLeft, MessageSquare, BookOpen, Terminal, MousePointer2, Play, Settings, Save, Zap, Settings2, FileCode, Scissors, Database, Wand2 } from 'lucide-react';
...
const RightSidebar: React.FC = () => {
    ...
    const { 
        selectedCommandId, commands, updateCommand, rightSidebarTab, setRightSidebarTab,
        editingCommand, setEditingCommand, setPendingParamCommand,
        addToast
    } = useStore();
    ...
    const handleSave = () => {
        if (selectedCommandId && editingCommand) {
            ...
            addToast('success', t('toast.saved'), `Command "${editingCommand.name}" updated.`);
        }
    };
    ...
    return (
        ...
        >
            ...
            {/* Content Area */}
            <div className={cn("flex-1 overflow-hidden flex flex-col bg-background/50 transition-opacity duration-200 relative", isCollapsed ? "opacity-0 pointer-events-none" : "opacity-100")}>
                ...
                {isEditorTab && selectedCommand && (
                    <div className="flex items-center justify-between p-3 border-b border-border bg-background shrink-0">
                        <div className="flex items-center gap-2 overflow-hidden">
                            ...
                            <span className="font-bold text-sm truncate">{editingCommand?.name || selectedCommand.name}</span>
                            <span className="text-[10px] font-mono text-muted-foreground opacity-50 uppercase">{rightSidebarTab}</span>
                        </div>
                    </div>
                )}
                ...
                {isEditorTab && selectedCommand && (
                    <div className="p-3 border-t border-border bg-background flex justify-between gap-3 shrink-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                        <Button variant="outline" onClick={handleSave} className="flex-1 gap-2" title="Save changes to library">
                            <Save className="w-4 h-4" /> {t('modal.save')}
                        </Button>
                        <Button onClick={handleSend} className="flex-1 gap-2 bg-emerald-600 hover:bg-emerald-700 text-white shadow-md" title="Run with current settings (Saved or Unsaved)">
